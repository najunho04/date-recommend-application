...
public class ViewMyLikesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_view_my_likes);
            ...
            switch (actionType) {
                ...
                case RETWEET:
                    if(post.getParentPostId() != null){
                        //자신이 자식 post일 때
                        Toast.makeText(ViewMyLikesActivity.this, "이미 리트윗 된 게시물입니다.", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    if(mAuth.getCurrentUser().toString().equals(post.getCreatedBy())){
                        //자기 게시물 리트윗 시도 시
                        Toast.makeText(ViewMyLikesActivity.this, "본인이 작성한 게시물은 리트윗 불가합니다.", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    Intent intent = new Intent(ViewMyLikesActivity.this, RetweetActivity.class);
    ...
    protected void onStart(){
        super.onStart();
        userRepo.getMyLikesPostsId(mAuth.getCurrentUser().getUid(), new UserRepo.OnGetMyPostsListener() {
            @Override
            public void onSuccess(List<String> postsId) {
                viewModel.getPostsInUser(postsId, new MainViewModel.OnPostsListener(){
                    @Override
                    public void onSuccess(List<Post> posts) {
                        Log.d("getMyLikesPostsId", "success: "+posts.size());
                        adapter.submitList(posts);
                    }

                    @Override
                    public void onError(String errorMessage) {
                        Log.d("getMyLikesPostsId", "error: "+errorMessage);
                    }
                });
            }
    ...
    private void openPost(Post post) {
        Intent intent = new Intent(ViewMyLikesActivity.this, OpenPostActivity.class);
