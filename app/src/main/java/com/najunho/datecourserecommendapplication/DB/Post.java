...
public class Post implements Serializable {
    ...
    private boolean likePending = false;
    ...
    public Post(Post source) {
        ...
        // Timestamp는 mutable → 새 객체 생성
        this.createdAt = source.createdAt != null
                ? new Timestamp(source.createdAt.toDate())
                : null;
        ...
        // List는 반드시 새 ArrayList로 복사
        // null-safe deep copy
        this.likesBy = source.likesBy != null
                ? new ArrayList<>(source.likesBy)
                : new ArrayList<>();

        this.retweetBy = source.retweetBy != null
                ? new ArrayList<>(source.retweetBy)
                : new ArrayList<>();

        this.postInterests = source.postInterests != null
                ? new ArrayList<>(source.postInterests)
                : new ArrayList<>();

        this.parentPostId = source.parentPostId;
        this.isRetweeted = source.isRetweeted;
        this.thumbnail = source.thumbnail;

        this.score = source.score;
        this.coreRegion = source.coreRegion;
        this.coreLatitude = source.coreLatitude;
        this.coreLongitude = source.coreLongitude;
    }
    ...
    public boolean getLikePending() {
        return likePending;
    }

    public void setLikePending(boolean likePending) {
